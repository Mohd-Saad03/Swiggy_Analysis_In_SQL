...
select city,avg(rating) as rating_avg
from swiggy
group by city;
...
select distinct restaurant_name,count(case when veg_or_nonveg = 'Veg' then 1 end)*100/count(*) as vegiterian_percent
from swiggy
group by restaurant_name
having vegiterian_percent = 100
...
select distinct restaurant_name,count(case when veg_or_nonveg = 'Non-veg' then 1 end)*100/count(*) as percentage
from swiggy
group by restaurant_name
...
with ratingbycity as (
select
distinct restaurant_name,city,rating,
dense_rank() over(partition by city order by rating desc) as rating_rnk
from swiggy)
select restaurant_name,city,rating,rating_rnk
from ratingbycity
where rating_rnk =1;
